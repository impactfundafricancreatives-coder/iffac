  ...
  return (
    <div className="bg-[#F4EFE6]">
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden" style={{ minHeight: "60vh" }}>
        <div className="absolute inset-0 z-0">
          <Image
            ...
            priority
          />
          <div className="absolute inset-0" style={{ background: "linear-gradient(105deg, rgba(13,27,42,0.96) 0%, rgba(13,27,42,0.85) 55%, rgba(13,27,42,0.5) 100%)" }} />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-16 lg:px-24">
          <p className="text-[#B8902A] text-xs font-bold tracking-[0.2em] uppercase mb-6">Programs</p>
          ...
          <p className="text-[#C8C4BC] text-lg md:text-xl max-w-2xl leading-relaxed mb-10">
            Built for how creative businesses actually grow. Enter at any stage, exit with global reach.
          </p>
          <Link
            href="/apply"
            className="inline-flex items-center bg-[#B8902A] text-[#0D1B2A] text-sm font-bold px-8 py-4 hover:bg-[#D4A843] transition-colors"
          >
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 lg:px-24 border-b border-[#A8A39A]/20">
        <div className="max-w-7xl mx-auto space-y-px bg-[#A8A39A]/20">
          {stages.map((s) => (
            <div key={s.n} className="bg-[#F4EFE6] p-8 md:p-12 grid grid-cols-1 lg:grid-cols-3 gap-10">
              {/* Left */}
              <div className="lg:col-span-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="font-display text-[#0B1B2B] text-2xl font-medium">{s.range}</p>
                  </div>
                </div>
                <p className="text-[#4A4A4A] text-sm">{s.duration}</p>
              </div>

              {/* Middle */}
              <div className="lg:col-span-1">
                <p className="text-[#A8A39A] text-xs font-bold uppercase tracking-wide mb-3">What you get</p>
                <ul className="space-y-2">
                  {s.get.map((g) => (
                    <li key={g} className="flex items-start gap-2 text-[#0B1B2B] text-sm">
                      <span className="mt-1 shrink-0 w-1.5 h-1.5 rounded-full bg-[#C9A24A]" />
                      {g}
                    </li>
                  ))}
                </ul>
              ...
              {/* Right */}
              <div className="lg:col-span-1">
                <p className="text-[#A8A39A] text-xs font-bold uppercase tracking-wide mb-3">Eligibility</p>
                <p className="text-[#4A4A4A] text-sm leading-relaxed mb-6">{s.eligibility}</p>
                <Link
                  href="/apply"
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 lg:px-24 bg-[#0B1B2B]">
        <div className="max-w-7xl mx-auto">
          <p className="text-[#C9A24A] text-xs font-bold tracking-[0.2em] uppercase mb-5">Funding Pillars</p>
          <h2 className="font-display text-white text-4xl font-medium mb-14">Four vehicles. One mandate.</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10">
            {pillars.map((p) => (
              <div key={p.name} className="bg-[#0B1B2B] p-10 hover:bg-[#0f2338] transition-colors">
                <p className="text-[#C9A24A] text-xl font-display font-medium mb-2">{p.amount}</p>
      ...
      <section className="py-20 px-6 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-4">
          <Link href="/apply" className="inline-flex items-center justify-center bg-[#0B1B2B] text-white text-sm font-bold px-8 py-4 hover:bg-[#111111] transition-colors">
            Apply now →
          </Link>
          <Link href="/portfolio" className="inline-flex items-center justify-center border border-[#0B1B2B] text-[#0B1B2B] text-sm font-semibold px-8 py-4 hover:bg-[#0B1B2B] hover:text-white transition-colors">
            See the portfolio
          </Link>
        </div>
      </section>
