...
const pillars = [
  { acronym: "BHTG", name: "Black Heritage Textiles Group", amount: "US$100M", desc: "Circularity and value-chain enablement in African textiles and manufacturing.", color: "#2F5D3A" },
  ...
  { acronym: "Film", name: "Film Sub-Fund", amount: "US$1M", desc: "Strategic investment into TalentX's $2M fund, backing African cinema.", color: "#B5651D" },
  { acronym: "AFFI", name: "Grants for Upskilling", amount: "US$22M", desc: "Technical assistance including the African Fashion Futures Incubator.", color: "#C9A24A" },
];
    ...
    <div className="bg-[#F4EFE6]">
      ...
      <section className="pt-32 pb-20 md:pt-40 md:pb-28 bg-[#0B1B2B] px-6 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto">
          <p className="text-[#C9A24A] text-xs font-bold tracking-[0.2em] uppercase mb-6">For Investors</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a href="#data-room" className="inline-flex items-center justify-center bg-[#C9A24A] text-[#0B1B2B] text-sm font-bold px-8 py-4 hover:bg-[#b8913e] transition-colors">
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 lg:px-24 border-b border-[#A8A39A]/20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            <p className="text-[#B5651D] text-xs font-bold tracking-[0.2em] uppercase mb-5">The Thesis</p>
            <h2 className="font-display text-[#0B1B2B] text-4xl font-medium leading-snug mb-6">
          ...
          <div className="grid grid-cols-2 gap-px bg-[#A8A39A]/20">
            {[
              ...
              <div key={s.l} className="bg-[#F4EFE6] p-8">
                <p className="font-display text-[#0B1B2B] text-4xl font-medium mb-2">{s.v}</p>
                <p className="text-[#A8A39A] text-xs leading-relaxed">{s.l}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 lg:px-24 bg-[#0B1B2B]">
        <div className="max-w-7xl mx-auto">
          <p className="text-[#C9A24A] text-xs font-bold tracking-[0.2em] uppercase mb-5">Fund Structure</p>
          <h2 className="font-display text-white text-4xl font-medium mb-5">Four funding pillars.</h2>
          <p className="text-[#A8A39A] text-lg max-w-xl mb-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10">
            {pillars.map((p) => (
              <div key={p.acronym} className="bg-[#0B1B2B] p-10 hover:bg-[#0f2338] transition-colors border-t-2" style={{ borderColor: p.color }}>
                ...
                <p className="text-[#A8A39A] text-sm leading-relaxed">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 lg:px-24 border-b border-[#A8A39A]/20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            <p className="text-[#B5651D] text-xs font-bold tracking-[0.2em] uppercase mb-5">Governance & ESG</p>
            <h2 className="font-display text-[#0B1B2B] text-4xl font-medium leading-snug mb-6">
            ...
            <Link href="/about" className="text-[#0B1B2B] text-sm font-bold hover:text-[#B8902A] transition-colors">
          ...
          <div>
            <div className="mb-8 space-y-0">
              {[
                ...
                <div key={row.label} className="flex justify-between py-4 border-b border-[#A8A39A]/20">
                  <span className="text-[#A8A39A] text-sm">{row.label}</span>
                  <span className="text-[#0B1B2B] text-sm font-semibold text-right max-w-[200px]">{row.value}</span>
                </div>
              ))}
            </div>

            <p className="text-[#A8A39A] text-xs font-bold uppercase tracking-wide mb-4">UN SDG Alignment</p>
            <div className="grid grid-cols-2 gap-3">
              {sdgs.map((s) => (
                <div key={s.n} className="flex items-center gap-3 border border-[#A8A39A]/20 p-4 bg-white">
                  <span className="font-display text-[#0B1B2B] text-3xl font-medium leading-none">{s.n}</span>
                  <span className="text-[#4A4A4A] text-xs leading-tight">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Network photo */}
      <section className="py-20 md:py-24 px-6 md:px-16 lg:px-24 border-b border-[#A8A39A]/20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            <p className="text-[#B5651D] text-xs font-bold tracking-[0.2em] uppercase mb-5">Our Investor Network</p>
      ...
      {/* Data room request */}
      <section id="data-room" className="py-20 md:py-28 bg-[#0B1B2B] px-6 md:px-16 lg:px-24">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            <p className="text-[#C9A24A] text-xs font-bold tracking-[0.2em] uppercase mb-5">Investor Relations</p>
            <h2 className="font-display text-white text-4xl font-medium leading-snug mb-5">
              Ready to review the full package?
            </h2>
            <p className="text-[#A8A39A] text-base leading-relaxed mb-6">
            ...
            <p className="text-[#4A4A4A] text-sm">We respond to all qualified inquiries within 2 business days.</p>
          </div>
          <DataRoomForm />
        </div>
      </section>
